 ...
 public:
  ...
  class Iterator {
   public:
    ...
    Iterator<CB>& operator=(const Iterator<CB>& other) {data_ = other.data_; i_ = other.i_; return *this;}
    Iterator<CB>& operator+=(const difference_type i) {i_ += i; return *this;}
    Iterator<CB>& operator-=(const difference_type i) {i_ -= i; return *this;}
    const T& operator*() const {return (*data_)[i_];}
    const T* operator->() const {return &(*data_)[i_];}
    const T& operator[](const difference_type i) const {return (*data_)[i_+i];}
    
    Iterator<CB>& operator++() {++i_; return *this;}
    Iterator<CB> operator++(int) {auto out = *this; ++*this; return out;}
    Iterator<CB>& operator--() {--i_; return *this;}
    Iterator<CB> operator--(int) {auto out = *this; --*this; return out;}
    Iterator<CB> operator+(const Iterator<CB>& rhs) const {return Iterator<CB>(*data_, i_+rhs.i_);}
    Iterator<CB> operator-(const Iterator<CB>& rhs) const {return Iterator<CB>(*data_, i_-rhs.i_);}
    Iterator<CB> operator+(const difference_type& rhs) const {return Iterator<CB>(*data_, i_+rhs);}
    Iterator<CB> operator-(const difference_type& rhs) const {return Iterator<CB>(*data_, i_-rhs);}
    
    bool operator==(const Iterator<CB>& other) const {return i_ == other.i_;}
    bool operator!=(const Iterator<CB>& other) const {return !(*this == other);}
    bool operator<(const Iterator<CB>& other) const {return i_ < other.i_;}
    bool operator>(const Iterator<CB>& other) const {return i_ > other.i_;}
    bool operator<=(const Iterator<CB>& other) const {return i_ <= other.i_;}
    bool operator>=(const Iterator<CB>& other) const {return i_ >= other.i_;}
  ...
  const Iterator<const CircularBuffer<T>> begin() const {return Iterator(*this, 0);}

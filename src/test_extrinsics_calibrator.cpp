...
Eigen::Affine3f DistortTransformation(const Eigen::Affine3f &T, std::mt19937 &gen, const float translation_error, const float rotation_error_deg) {
  std::uniform_real_distribution<float> rand_trans_err(-translation_error, translation_error);
  std::uniform_real_distribution<float> rand_rot_angle_err(-rotation_error_deg, rotation_error_deg);

  auto T_distorted = T;

  constexpr auto pi = 3.141592653589793f;
  const auto R_err = Eigen::AngleAxisf(rand_rot_angle_err(gen) / 180.0f * pi, Eigen::Vector3f::UnitZ()) 
                        * Eigen::AngleAxisf(rand_rot_angle_err(gen) / 180.0f * pi, Eigen::Vector3f::UnitY()) 
                        * Eigen::AngleAxisf(rand_rot_angle_err(gen) / 180.0f * pi, Eigen::Vector3f::UnitZ());
...
//int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < num_cams; ++i)
  {
    ...
    cam_T_rig.translation() = Eigen::Vector3f{rand_trans_rig(gen), rand_trans_rig(gen), 0.0f};
    camera_T_rigs.push_back(cam_T_rig);
  }

  std::vector<Eigen::Affine3f, Eigen::aligned_allocator<Eigen::Affine3f>> camera_T_rigs_distorted;
  ...
  for (int i_frame = 0; i_frame < num_frames; ++i_frame)
  {
    ...
    {
      const Eigen::Vector4f point3d(rand_point_trans(gen),rand_point_trans(gen),rand_point_trans(gen), 1.0f);
      ...
      {
        Eigen::Vector4f point3d_cam = camera_T_rigs[i_cam] * rig_T_world * point3d;
        Eigen::Vector2f point_2d(point3d_cam(0)/point3d_cam(2), point3d_cam(1)/point3d_cam(2));
